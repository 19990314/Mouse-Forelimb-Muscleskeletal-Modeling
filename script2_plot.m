...
%% ===== LOAD RESULTS =====
if nargin < 1
    matfile = 'MouseForelimb[input]_montecarlo_muscle_results_v2.mat';
end
...
%% ===== FIGURE 1: VELOCITY DYNAMICS (PRIMARY) =====
figure('Position', [100 100 1400 600], 'Name', 'Muscle Velocity Dynamics - Primary Representatives', 'Color', 'w');
...
%% ===== NEW FIGURE: TOP 10 MUSCLE VELOCITY COMPARISON =====
figure('Position', [120 80 1800 900], 'Name', 'Muscle Velocity - Top 10 Trajectories', 'Color', 'w');

% Create tiled layout for better control
tiledlayout(2, nTop+1, 'TileSpacing', 'compact', 'Padding', 'compact');

% 5 MOST contrasting (top row)
for i = 1:nTop
    nexttile;
    hold on;
    
    Vm_i = Vm_norm_top5_most{i};
    for m = 1:nm
        plot(time, Vm_i(:,m), 'Color', colors(m,:), 'LineWidth', 1.2);
    end
    ...
    ylabel('V_{norm}', 'FontSize', 9);
    title(sprintf('MOST #%d (T%d)', i, top5_most_trials(i)), ...
          'FontSize', 10, 'FontWeight', 'bold', 'Color', red_colors(i,:));
    grid on;
    hold off;
end

% Legend in dedicated tile (top row, last position)
ax_legend = nexttile;
axis(ax_legend, 'off');
hold(ax_legend, 'on');
h_legend = [];
labels_legend = {};
for m = 1:nm
    h = plot(ax_legend, NaN, NaN, 'Color', colors(m,:), 'LineWidth', 2);
    h_legend(end+1) = h;
    labels_legend{end+1} = strrep(cfg.muscles{m}, '_', ' ');
end
legend(h_legend, labels_legend, 'Location', 'west', 'FontSize', 8, ...
       'Interpreter', 'none', 'Box', 'on');
title(ax_legend, 'Muscles', 'FontSize', 10, 'FontWeight', 'bold');
hold(ax_legend, 'off');

% 5 LEAST contrasting (bottom row)
for i = 1:nTop
    nexttile;
    hold on;
    
    Vm_i = Vm_norm_top5_least{i};
    for m = 1:nm
        plot(time, Vm_i(:,m), 'Color', colors(m,:), 'LineWidth', 1.2);
    end
    ...
    xlabel('Time (s)', 'FontSize', 9);
    ylabel('V_{norm}', 'FontSize', 9);
    title(sprintf('LEAST #%d (T%d)', i, top5_least_trials(i)), ...
          'FontSize', 10, 'FontWeight', 'bold', 'Color', blue_colors(i,:));
    grid on;
    hold off;
end

% Info panel in bottom-right tile
ax_info = nexttile;
axis(ax_info, 'off');
info_text = {
    '\bfColor Coding:'
    ...
    ''
    '\bfMetric:'
    'V_{norm} = V_{fiber} / V_{max}'
};
text(ax_info, 0.1, 0.9, info_text, 'FontSize', 9, 'VerticalAlignment', 'top', ...
     'Units', 'normalized', 'Interpreter', 'tex');

% Overall title
sgtitle('Muscle Velocity Profiles: Top 5 MOST (top row) vs Top 5 LEAST (bottom row)', ...
        'FontSize', 14, 'FontWeight', 'bold');

%% ===== FIGURE 2: 3D JOINT CONFIGURATION WITH LABELS =====
...
if ~isempty(shoulder_idx) && ~isempty(elbow_idx) && ~isempty(wrist_idx)
    ...
    for i = nTop:-1:1
        ...
        plot3(sh(end), el(end), wr(end), 'o', 'Color', red_colors(i,:), ...
              'MarkerSize', 8+2*(nTop-i), 'MarkerFaceColor', red_colors(i,:), ...
              'HandleVisibility', 'off');
        
        % Label at end point
        text(sh(end), el(end), wr(end), sprintf(' T%d', top5_most_trials(i)), ...
             'FontSize', 9, 'FontWeight', 'bold', 'Color', red_colors(i,:));
    end
    
    % Plot TOP 5 LEAST with labels
    for i = nTop:-1:1
        Q_i = Q_top5_least{i};
        sh = rad2deg(Q_i(:, shoulder_idx));
        el = rad2deg(Q_i(:, elbow_idx));
        ...
        plot3(sh(end), el(end), wr(end), 's', 'Color', blue_colors(i,:), ...
              'MarkerSize', 8+2*(nTop-i), 'MarkerFaceColor', blue_colors(i,:), ...
              'HandleVisibility', 'off');
        
        % Label at end point
        text(sh(end), el(end), wr(end), sprintf(' T%d', top5_least_trials(i)), ...
             'FontSize', 9, 'FontWeight', 'bold', 'Color', blue_colors(i,:));
    end
    ...
    plot3(sh_start, el_start, wr_start, 'kp', 'MarkerSize', 16, ...
          'MarkerFaceColor', 'y', 'LineWidth', 2, 'DisplayName', 'Start');
    
    xlabel([strrep(cfg.coordNames{shoulder_idx}, '_', ' ') ' (deg)'], ...
           'FontSize', 12, 'FontWeight', 'bold');
    ylabel('Elbow Flex (deg)', 'FontSize', 12, 'FontWeight', 'bold');
    zlabel('Wrist Angle (deg)', 'FontSize', 12, 'FontWeight', 'bold');
    title(sprintf('3D Joint Configuration: Top %d MOST (red) vs LEAST (blue) with Trial Numbers', nTop), ...
          'FontSize', 14);
    grid on; grid minor;
    view(135, 25);
...
%% ===== FIGURE 3: 3D HAND TRAJECTORY WITH LABELS =====
...
% Plot TOP 5 MOST contrasting with labels
for i = nTop:-1:1
    ...
    % Label at end
    text(ax, xyz_head(end,1), xyz_head(end,2), xyz_head(end,3), ...
         sprintf(' T%d', top5_most_trials(i)), 'FontSize', 9, ...
         'FontWeight', 'bold', 'Color', red_colors(i,:));
end

% Plot TOP 5 LEAST contrasting with labels
for i = nTop:-1:1
    ...
    % Label at end
    text(ax, xyz_head(end,1), xyz_head(end,2), xyz_head(end,3), ...
         sprintf(' T%d', top5_least_trials(i)), 'FontSize', 9, ...
         'FontWeight', 'bold', 'Color', blue_colors(i,:));
end

% HEAD at origin
plot3(ax, 0, 0, 0, 'ko', 'MarkerFaceColor', [0.3 0.3 0.3], 'MarkerSize', 20, ...
      'LineWidth', 2, 'DisplayName', 'Head (origin)');
text(ax, 0, 3, 0, 'HEAD', 'FontSize', 11, 'FontWeight', 'bold', ...
     'HorizontalAlignment', 'center');

% Hand START position
plot3(ax, head_to_hand_start, 0, 0, 'kp', 'MarkerFaceColor', 'y', ...
      'MarkerSize', 14, 'LineWidth', 2, 'DisplayName', 'Hand Start');

% Body line
plot3(ax, [0, head_to_hand_start], [0, 0], [0, 0], 'k-', 'LineWidth', 2, ...
      'HandleVisibility', 'off');

% Coordinate axes
axis_length = 10;
quiver3(ax, 0, 0, 0, axis_length, 0, 0, 0, 'Color', [0.8 0 0], ...
        'LineWidth', 3, 'MaxHeadSize', 0.3, 'HandleVisibility', 'off');
text(ax, axis_length+2, 0, 0, '+X (Reach)', 'FontSize', 11, ...
     'FontWeight', 'bold', 'Color', [0.8 0 0]);
quiver3(ax, 0, 0, 0, 0, axis_length, 0, 0, 'Color', [0 0.7 0], ...
        'LineWidth', 3, 'MaxHeadSize', 0.3, 'HandleVisibility', 'off');
text(ax, 0, axis_length+2, 0, '+Y (Up)', 'FontSize', 11, ...
     'FontWeight', 'bold', 'Color', [0 0.7 0]);
quiver3(ax, 0, 0, 0, 0, 0, axis_length, 0, 'Color', [0 0 0.8], ...
        'LineWidth', 3, 'MaxHeadSize', 0.3, 'HandleVisibility', 'off');
text(ax, 0, 0, axis_length+2, '+Z (Right)', 'FontSize', 11, ...
...
title(ax, {'Hand Trajectories with Trial Numbers (Head at Origin)'; ...
...
ax2 = subplot(1,2,2);
axis(ax2, 'off');
text(ax2, 0.5, 0.98, 'HEAD-CENTERED COORDINATES', 'FontSize', 16, ...
...
xlabel('Time (s)');
ylabel('\Delta L (mm)');
title(sprintf('Top %d Muscles Length Change (Trial #%d)', ...
      nTopMuscles, most_contrast_trial));
...
hold on;
for m = 1:nm
    bar(m, dL_net_mm(m), 'FaceColor', barColors(m,:), 'EdgeColor', 'none', ...
        'BarWidth', 0.8);
end
hold off;
xlabel('Muscle');
...
title({'Joint Excursion Comparison'; ...
       sprintf('MOST (T%d): %.1f° total vs LEAST (T%d): %.1f° total', ...
       most_contrast_trial, sum(abs(delta_Q_most)), ...
       least_contrast_trial, sum(abs(delta_Q_least)))}, 'FontSize', 11);
legend('MOST', 'LEAST', 'Location', 'northeast');
grid on;

subplot(1,2,2);
axis off;
text(0.5, 0.9, 'Summary', 'FontSize', 14, 'FontWeight', 'bold', ...
     'HorizontalAlignment', 'center');
summary_text = {
    ''
    ...
    ''
    sprintf('Top 5 MOST trial numbers: %s', mat2str(top5_most_trials))
    sprintf('Top 5 LEAST trial numbers: %s', mat2str(top5_least_trials))
};
